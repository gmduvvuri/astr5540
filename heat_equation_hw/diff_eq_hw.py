...
def backward_euler_step(func, x_old, y_old, step, tol=0.001, max_iter=1000):
    x_new = x_old + step
    y_guess = euler_step(func, x_old, y_old, step)
    error = 0.1
    counter = 0
    while error > tol:
        y_new = y_old + step*func(x_new, y_guess)
        error = np.abs(y_new - y_guess)/y_guess
        y_guess = y_new
        counter += 1
        if counter > max_iter:
